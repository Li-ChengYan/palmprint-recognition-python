...
class MTCCAlgorithm:
    """Multi-order texture competitive code extractor and matcher."""
    ...
    def extract(self, image: np.ndarray) -> np.ndarray:
        ...
        for kernel in self.kernels:
            response_first = convolve2d(image_arr, kernel, mode="same", boundary="fill", fillvalue=0)
            response_second = convolve2d(response_first, kernel, mode="same", boundary="fill", fillvalue=0)
            features.append(response_first[::4, ::4] > 0)
            features.append(response_second[::4, ::4] > 0)
